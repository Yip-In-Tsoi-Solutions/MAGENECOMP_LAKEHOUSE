...
with DAG(
    dag_id="VW_DATA_CON_TRANSACTION_TEST",
    default_args=default_args,
    description='RUN Spark create VW_DATA_CON_TRANSACTION_TEST every 2 AM',
    schedule_interval='0 2 * * *',
    catchup=False,
) as dag:
    VW_DATA_CON_TRANSACTION_TEST = EzSparkSubmitOperator(
        spark_binary='spark-submit',
        application=f"s3a://notebooks/lakehouse/mart/DTC/VW_DATA_CON_TRANSACTION_TEST.py",
        conn_id='spark_conn',
        task_id='VW_DATA_CON_TRANSACTION_TEST',
        conf=conf,
        dag=dag
    )
