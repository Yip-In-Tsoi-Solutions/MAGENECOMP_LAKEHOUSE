...
with DAG(
    dag_id="PROCESS_FACT_DATA_CON_TRANSACTION_DAILY",
    default_args=default_args,
    description='FACT_DATA_CON_TRANSACTION_DAILY for daily',
    schedule_interval="0 15 * * *",
    catchup=False,
) as dag:
    FACT_DATA_CON_TRANSACTION = EzSparkSubmitOperator(
        spark_binary='spark-submit',
        application=f"s3a://notebooks/lakehouse/process/DTC/FACT_DATA_CON_TRANSACTION_daily.py",
        conn_id='spark_conn',
        task_id='FACT_DATA_CON_TRANSACTION_daily',
        conf=conf,
        dag=dag
    )
    VIEW_DATA_CON_TRANSACTION = EzSparkSubmitOperator(
        spark_binary='spark-submit',
        application=f"s3a://notebooks/lakehouse/mart/DTC/VW_DATA_CON_TRANSACTION_daily.py",
        conn_id='spark_conn',
        task_id='VIEW_DATA_CON_TRANSACTION_daily',
        conf=conf,
        dag=dag
    )
    FACT_DATA_CON_TRANSACTION >> VIEW_DATA_CON_TRANSACTION