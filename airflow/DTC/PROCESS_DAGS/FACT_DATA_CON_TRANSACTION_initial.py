...
with DAG(
    dag_id="PROCESS_FACT_DATA_CON_TRANSACTION_INITIAL",
    ...
    FACT_DATA_CON_TRANSACTION = EzSparkSubmitOperator(
        ...
        application=f"s3a://notebooks/lakehouse/process/DTC/FACT_DATA_CON_TRANSACTION_initial.py",
        conn_id='spark_conn',
        task_id='FACT_DATA_CON_TRANSACTION_initial',
        conf=conf,
        dag=dag
    )
    VIEW_DATA_CON_TRANSACTION = EzSparkSubmitOperator(
        spark_binary='spark-submit',
        application=f"s3a://notebooks/lakehouse/mart/DTC/VW_DATA_CON_TRANSACTION_initial.py",
        conn_id='spark_conn',
        task_id='VIEW_DATA_CON_TRANSACTION_initial',
        conf=conf,
        dag=dag
    )
