...
with DAG(
    dag_id="VW_DATA_CON_TRANSACTION",
    default_args=default_args,
    description='RUN Spark process VW_DATA_CON_TRANSACTION every 2 AM',
    schedule_interval='0 2 * * *',
    catchup=False,
    tags=['DTC', 'view','daily'],
) as dag:
    VW_DATA_CON_TRANSACTION = EzSparkSubmitOperator(
        spark_binary='spark-submit',
        application=f"s3a://notebooks/lakehouse/mart/DTC/VW_DATA_CON_TRANSACTION.py",
        conn_id='spark_conn',
        task_id='VW_DATA_CON_TRANSACTION',
        conf=conf,
        dag=dag
    )
