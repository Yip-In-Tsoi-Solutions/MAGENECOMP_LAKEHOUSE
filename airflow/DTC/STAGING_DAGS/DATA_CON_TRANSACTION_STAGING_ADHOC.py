...
with DAG(
    dag_id="DATA_CON_TRANSACTION_STAGING_ADHOC",
    default_args=default_args,
    description='Rerun Staging DTC with parameter dtc_num and dtc_file_date',
    schedule_interval=None,
    catchup=False,
    tags=['DTC', 'rerun','staging'],
) as dag:
    STAGING_DTC_TRANSACTION_ADHOC = EzSparkSubmitOperator(
        spark_binary='spark-submit',
        application=f"s3a://notebooks/lakehouse/staging/DTC/adhoc/STAGING_ALL_DTC_TRANSACTION_ADHOC.py",
        conn_id='spark_conn',
        task_id='STAGING_DTC_TRANSACTION_ADHOC',
        conf=conf,
        dag=dag
    )
    STAGING_DTC_TRANSACTION_ADHOC
