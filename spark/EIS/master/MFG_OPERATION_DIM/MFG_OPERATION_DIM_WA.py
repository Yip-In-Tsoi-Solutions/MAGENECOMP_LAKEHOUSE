...
df = spark.sql(f"""
   SELECT 
       ...
       'EIS_OPERATION_WA' as SOURCE_TABLE_NAME,
       {inf_batch_id} as INF_BATCH_ID,
       'master_MFG_OPERATION_DIM_WA' as INF_SESS_NAME,
...
""")
df.createOrReplaceTempView("MFG_OPERATION_DIM_WA")

count_insert_df = spark.sql(f"""
    SELECT count(1) as total_count
    FROM MFG_OPERATION_DIM_WA s
...
spark.sql(f"""
    WITH max_sequence AS (
        SELECT NVL(MAX(OPERATION_NO_KEY),100000000) AS max_seq FROM {master_table} WHERE PLANT='WA'
    )
    MERGE INTO {master_table} t 
    USING (SELECT * FROM MFG_OPERATION_DIM_WA) s 
    ON t.OPERATION_NO = s.OPERATION_NO and t.PLANT = s.PLANT
    WHEN NOT MATCHED
