...
df = spark.sql(f"""
   SELECT 
       ...
       'EIS_OPERATION_WH' as SOURCE_TABLE_NAME,
       {inf_batch_id} as INF_BATCH_ID,
       'master_MFG_OPERATION_DIM_WH' as INF_SESS_NAME,
...
""")
df.createOrReplaceTempView("MFG_OPERATION_DIM_WH")

count_insert_df = spark.sql(f"""
    SELECT count(1) as total_count
    FROM MFG_OPERATION_DIM_WH s
...
spark.sql(f"""
    WITH max_sequence AS (
        SELECT NVL(MAX(OPERATION_NO_KEY),300000000) AS max_seq FROM {master_table} WHERE PLANT='WH'
    )
    MERGE INTO {master_table} t 
    USING (SELECT * FROM MFG_OPERATION_DIM_WH) s 
    ON t.OPERATION_NO = s.OPERATION_NO and t.PLANT = s.PLANT
    WHEN NOT MATCHED
