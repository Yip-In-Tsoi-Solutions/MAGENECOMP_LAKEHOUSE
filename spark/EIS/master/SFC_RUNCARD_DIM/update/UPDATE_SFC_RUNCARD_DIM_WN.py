...
appName = "UPDATE_SFC_RUNCARD_DIM_WN"
...
df_runcard_dim = spark.table("master.lakehouse.DIM.SFC_RUNCARD_DIM").alias("rd").filter(F.col("PLANT") == 'WN').select(F.col("RUNCARD_NO_KEY"),F.col("RUNCARD_NO"))
df_run_convert = spark.table("staging.lakehouse.EIS.EIS_RUNCARD_CONVERT_WN") \
...
spark.sql(f"""
    ...
    FROM (
        SELECT b.RUNCARD_NO_KEY,b.original_runcard_no,b.current_run_no
              ...
              ,a.JIT_CELL_NO 
              ,'WN' AS PLANT
        FROM staging.lakehouse.EIS.EIS_RUNCARD_CONVERT_WN a
        INNER JOIN current_runcard_tmp b
        ON a.RUN_NO = b.current_run_no
        INNER JOIN staging.lakehouse.EIS.EIS_DOCUMENT_EER_WN EER 
...
count_update_df = spark.sql(f"""
    ...
    ON t.RUNCARD_NO = s.original_runcard_no AND t.PLANT='WN'
...
spark.sql(f"""
    ...
    WHEN MATCHED AND t.PLANT='WN' THEN 
