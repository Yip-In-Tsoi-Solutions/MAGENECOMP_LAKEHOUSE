...
appName = "UPDATE_SFC_RUNCARD_DIM_WH"
...
df_runcard_dim = spark.table("master.lakehouse.DIM.SFC_RUNCARD_DIM").alias("rd").filter(F.col("PLANT") == 'WH').select(F.col("RUNCARD_NO_KEY"),F.col("RUNCARD_NO"))
df_run_convert = spark.table("staging.lakehouse.EIS.EIS_RUNCARD_CONVERT_WH") \
...
# spark.sql(f"""
#     select b.RUNCARD_NO_KEY,b.original_runcard_no,b.current_run_no,a.CONVERTED_DATE,a.EER_NO 
#     from staging.lakehouse.EIS.EIS_RUNCARD_CONVERT_WH a
...
spark.sql(f"""
    ...
    FROM (
        ...
        INNER JOIN current_runcard_tmp b
        ON a.RUN_NO = b.current_run_no
        INNER JOIN staging.lakehouse.EIS.EIS_DOCUMENT_EER_WH EER 
...
count_update_df = spark.sql(f"""
    ...
    ON t.RUNCARD_NO = s.original_runcard_no AND t.PLANT='WH'
...
spark.sql(f"""
    ...
    WHEN MATCHED AND t.PLANT='WH' THEN 
