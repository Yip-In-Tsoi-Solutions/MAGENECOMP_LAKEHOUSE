...
df = spark.read\
    ...
    .option("dbtable", "EIS.EIS_RUNCARD_CONVERT") \
    .option("user", oracleUser) \
    .option("password", oraclePass) \
    .option("driver", "oracle.jdbc.driver.OracleDriver") \
    .load()
df.createOrReplaceTempView("EIS_RUNCARD_CONVERT_tmp")

df = spark.sql(f"""
    SELECT 
        SS.RUN_NO RUNCARD_NO,
        ...
        'EIS_WIP_FA_RUNCARD_WN' as SOURCE_TABLE_NAME,
        {inf_batch_id} as INF_BATCH_ID,
        'master_SFC_RUNCARD_DIM_WN' as INF_SESS_NAME,
    ...
    FROM (
        ...
        FROM
        	staging.lakehouse.EIS.EIS_WIP_FA_RUNCARD_WN RUNCARD
        INNER JOIN staging.lakehouse.EIS.EIS_DOCUMENT_EER_WN eer ON
        	RUNCARD.EER_NO = EER.EERNO
        	AND RUNCARD.PRODUCT_PART_NO = EER.PRODUCT_PART_NO
        	AND RUNCARD.JIT_CELL_NO = EER.JIT_CELL_NO
        WHERE RUNCARD.CREATED_DATE BETWEEN cast(cast(date_add(current_date(),-1) as string) || ' 07:00:01' as timestamp)
...
""")
df.createOrReplaceTempView("SFC_RUNCARD_DIM_WN")

# print("Count record(Before Upsert) :",df.count())
count_insert_df = spark.sql(f"""
    SELECT count(1) as total_count
    FROM SFC_RUNCARD_DIM_WN s
...
# spark.sql(f"""
#     WITH tmp_join as (
#           SELECT * FROM staging.lakehouse.EIS.EIS_RUNCARD_CONVERT_WN
#           )
#     MERGE INTO {master_table} t
#     USING (SELECT * FROM staging.lakehouse.EIS.EIS_RUNCARD_CONVERT_WN) s
#     ON t.RUNCARD_NO = s.RUNCARD_NO and t.PRODUCT_PART_NO_KEY and t.PLANT = s.PLANT
#     WHEN MATCHED THEN 
#         UPDATE SET t.FROM_RUN = s.RUNCARD_NO
# """)

#Insert
# spark.sql(f"""
#     WITH max_sequence AS (
#         SELECT NVL(MAX(RUNCARD_NO_KEY),600000000) AS max_seq FROM {master_table} WHERE PLANT='WN'
#     )
#     MERGE INTO {master_table} t 
#     USING (SELECT * FROM SFC_RUNCARD_DIM_WN) s 
#     ON t.OPERATION_NO = s.OPERATION_NO and t.EER_NO = s.EER_NO and t.OPERATION_SEQ=s.ROUTING_SEQ and COALESCE(t.PRODUCT_PART_NO_KEY,0)=COALESCE(s.PRODUCT_PART_NO_KEY,0)
#     WHEN NOT MATCHED
#         THEN INSERT (RUNCARD_NO_KEY,REC_ID,SOURCE_TABLE_NAME,INF_BATCH_ID,INF_SESS_NAME,DM_MODIFIED_BY,DM_MODIFIED_DATE,DM_CREATED_BY,DM_CREATED_DATE,DM_DATE_FROM,DM_DATE_TO)
#         VALUES ((SELECT max_seq from max_sequence)+row_Number() over (ORDER BY s.OPERATION_NO),s.REC_ID,s.SOURCE_TABLE_NAME,s.INF_BATCH_ID,s.INF_SESS_NAME,s.DM_MODIFIED_BY,s.DM_MODIFIED_DATE,s.DM_CREATED_BY,s.DM_CREATED_DATE,s.DM_DATE_FROM,s.DM_DATE_TO)
# """)

# spark.sql(f"""
#     DELETE FROM {master_table} WHERE INF_BATCH_ID={inf_batch_id} AND INF_SESS_NAME='master_SFC_RUNCARD_DIM_WN'
# """)


# spark.sql(f"""
#     INSERT INTO {master_table} SELECT * FROM SFC_RUNCARD_DIM_WN
# """)
