...
df = spark.sql(f"""
    SELECT 
        ...
        'EIS_WIP_FA_RUNCARD_WA' as SOURCE_TABLE_NAME,
        {inf_batch_id} as INF_BATCH_ID,
        'master_SFC_RUNCARD_DIM_WA' as INF_SESS_NAME,
    ...
    FROM (
        ...
        FROM
        	staging.lakehouse.EIS.EIS_WIP_FA_RUNCARD_WA RUNCARD
        INNER JOIN staging.lakehouse.EIS.EIS_DOCUMENT_EER_WA eer ON
...
""")
df.createOrReplaceTempView("SFC_RUNCARD_DIM_WA")

count_insert_df = spark.sql(f"""
    SELECT count(1) as total_count
    FROM SFC_RUNCARD_DIM_WA s
...
# spark.sql(f"""
#     INSERT INTO {master_table} SELECT * FROM SFC_RUNCARD_DIM_WA
# """)

#Upsert data into SFC_RUNCARD_DIM
spark.sql(f"""
    WITH max_sequence AS (
        SELECT NVL(MAX(RUNCARD_NO_KEY),100000000) AS max_seq FROM {master_table} WHERE PLANT='WA'
    )
