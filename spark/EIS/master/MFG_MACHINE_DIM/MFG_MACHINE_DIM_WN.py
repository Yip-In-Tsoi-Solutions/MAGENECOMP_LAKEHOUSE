...
df = spark.sql(f"""
    SELECT 
        ...
        'master_MFG_MACHINE_DIM_WN' as INF_SESS_NAME,
    ...
    FROM staging.lakehouse.EIS.EIS_MACHINE_WN
"""
)
df.createOrReplaceTempView("MFG_MACHINE_DIM_WN")

count_insert_df = spark.sql(f"""
    SELECT count(1) as total_count
    FROM MFG_MACHINE_DIM_WN s
...
spark.sql(f"""
    WITH max_sequence AS (
        SELECT NVL(MAX(MACHINE_NO_KEY),600000000) AS max_seq FROM {master_table} WHERE INF_SESS_NAME='master_MFG_MACHINE_DIM_WN'
    )
    MERGE INTO {master_table} t 
    USING (SELECT * FROM MFG_MACHINE_DIM_WN) s 
    ON t.MACHINE_NO = s.MACHINE_NO
    WHEN NOT MATCHED
