...
df = spark.sql(f"""
    SELECT 
        ...
        'EIS_MACHINE_WH' as SOURCE_TABLE_NAME,
        {inf_batch_id} as INF_BATCH_ID,
        'master_MFG_MACHINE_DIM_WH' as INF_SESS_NAME,
    ...
    FROM staging.lakehouse.EIS.EIS_MACHINE_WH
"""
)
df.createOrReplaceTempView("MFG_MACHINE_DIM_WH")

count_insert_df = spark.sql(f"""
    SELECT count(1) as total_count
    FROM MFG_MACHINE_DIM_WH s
...
spark.sql(f"""
    WITH max_sequence AS (
        SELECT NVL(MAX(MACHINE_NO_KEY),300000000) AS max_seq FROM {master_table} WHERE INF_SESS_NAME='master_MFG_MACHINE_DIM_WH'
    )
    MERGE INTO {master_table} t 
    USING (SELECT * FROM MFG_MACHINE_DIM_WH) s 
    ON t.MACHINE_NO = s.MACHINE_NO
    WHEN NOT MATCHED
