...
df = spark.sql(f"""
    SELECT 
        ...
        'EIS_PRODUCT_OVERALL_WN' as SOURCE_TABLE_NAME,
        {inf_batch_id} as INF_BATCH_ID,
        'master_MFG_PRODUCT_DIM_WN' as INF_SESS_NAME,
    ...
    FROM staging.lakehouse.EIS.EIS_PRODUCT_OVERALL_WN a
    LEFT JOIN staging.lakehouse.EIS.EIS_PRODUCT_FAMILY_WN b 
        ON a.FAMILY_PRODUCT_ID = b.FAMILY_PRODUCT_ID
    LEFT JOIN staging.lakehouse.EIS.EIS_PRODUCT_PART_WN c 
        ON TRIM(a.PRODUCT_PART_NO) = c.PRODUCT_PART_NO
    WHERE
        b.STATUS = 'ACTIVE'
""")
df.createOrReplaceTempView("MFG_PRODUCT_DIM_WN")

count_update_df = spark.sql(f"""
    SELECT count(1) as total_count
    FROM {master_table} t 
    INNER JOIN MFG_PRODUCT_DIM_WN s 
...
spark.sql(f"""
    ...
    USING (SELECT * FROM tmp_product_wn) s
    ON t.PRODUCT_PART_NO = s.PRODUCT_PART_NO and t.PLANT = s.PLANT
