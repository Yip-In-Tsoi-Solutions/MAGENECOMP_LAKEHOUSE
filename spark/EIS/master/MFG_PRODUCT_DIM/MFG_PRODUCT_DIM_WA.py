...
spark = (
     ...
     .master("local")\
     .appName(appName)\
    .config('spark.jars.packages', 'org.apache.hadoop:hadoop-aws:3.3.4,org.apache.iceberg:iceberg-spark-runtime-3.3_2.12:1.1.0,software.amazon.awssdk:bundle:2.18.31,software.amazon.awssdk:url-connection-client:2.18.31')\
...
df = spark.sql(f"""
    SELECT 
        ...
        'EIS_PRODUCT_OVERALL_WA' as SOURCE_TABLE_NAME,
        {inf_batch_id} as INF_BATCH_ID,
        'master_MFG_PRODUCT_DIM_WA' as INF_SESS_NAME,
    ...
    FROM staging.lakehouse.EIS.EIS_PRODUCT_OVERALL_WA a
    LEFT JOIN staging.lakehouse.EIS.EIS_PRODUCT_FAMILY_WA b 
        ON a.FAMILY_PRODUCT_ID = b.FAMILY_PRODUCT_ID
    LEFT JOIN staging.lakehouse.EIS.EIS_PRODUCT_PART_WA c 
        ON TRIM(a.PRODUCT_PART_NO) = c.PRODUCT_PART_NO
    WHERE
        b.STATUS = 'ACTIVE'
""")
df.createOrReplaceTempView("MFG_PRODUCT_DIM_WA")

count_update_df = spark.sql(f"""
    SELECT count(1) as total_count
    FROM {master_table} t 
    INNER JOIN MFG_PRODUCT_DIM_WA s 
    ON t.PRODUCT_PART_NO = s.PRODUCT_PART_NO and t.PLANT = s.PLANT AND t.STATUS != s.STATUS
...
spark.sql(f"""
    ...
    USING (SELECT * FROM tmp_product_wa) s
    ON t.PRODUCT_PART_NO = s.PRODUCT_PART_NO and t.PLANT = s.PLANT
...
spark.sql(f"""
    WITH max_sequence AS (
        SELECT NVL(MAX(PRODUCT_PART_NO_KEY),100000000) AS max_seq FROM {master_table} WHERE PLANT='WA'
    )
    MERGE INTO {master_table} t 
    USING (SELECT * FROM MFG_PRODUCT_DIM_WA) s 
    ON t.PRODUCT_PART_NO = s.PRODUCT_PART_NO and t.PLANT = s.PLANT
    WHEN NOT MATCHED
