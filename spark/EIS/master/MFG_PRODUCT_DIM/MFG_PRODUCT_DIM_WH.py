...
count_update_df = spark.sql(f"""
    ...
    FROM {master_table} t 
    INNER JOIN MFG_PRODUCT_DIM_WH s 
...
spark.sql(f"""
    ...
    USING (SELECT * FROM tmp_product_wh) s
    ON t.PRODUCT_PART_NO = s.PRODUCT_PART_NO and t.PLANT = s.PLANT
...
spark.sql(f"""
    WITH max_sequence AS (
        SELECT NVL(MAX(PRODUCT_PART_NO_KEY),300000000) AS max_seq FROM {master_table} WHERE PLANT='WH'
    )
    MERGE INTO {master_table} t 
    USING (SELECT * FROM MFG_PRODUCT_DIM_WH) s 
    ON t.PRODUCT_PART_NO = s.PRODUCT_PART_NO and t.PLANT = s.PLANT
    WHEN NOT MATCHED
