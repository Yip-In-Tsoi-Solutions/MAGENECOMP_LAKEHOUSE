...
appName = "MFG_ROUTING_OPERATION_DIM_WH"
...
df = spark.sql(f"""      
   WITH opers AS (
        ...
        d.LAT_LOT_TYPE
        FROM staging.lakehouse.EIS.EIS_ROUTING_OPERATION_WH ro
        LEFT JOIN staging.lakehouse.EIS.EIS_OPERATION_WH o on ro.OPERATION_NO = o.OPERATION_NO
        LEFT JOIN staging.lakehouse.EIS.EIS_DOCUMENT_EER_WH d on ro.PRODUCT_PART_NO = d.PRODUCT_PART_NO and ro.EER_NO = d.EERNO
        LEFT JOIN (     
                SELECT  OPERATION_NO,
    ...
    SELECT 
        ...
        'EIS_ROUTING_OPERATION_WH' as SOURCE_TABLE_NAME,
        {inf_batch_id} as INF_BATCH_ID,
        'master_MFG_ROUTING_OPERATION_DIM_WH' as INF_SESS_NAME,
    ...
    BB.YIELD
    FROM(
        SELECT OPERATION_NO,PRODUCT_PART_NO ,max(EER_NO) EER_NO, max(OPERATION_NAME) OPERATION_NAME, ROUTING_SEQ, max(LAT_LOT_TYPE) LAT_LOT_TYPE,'WH' PLANT 
        FROM opers
    ...
    END AS YIELD
    FROM
    staging.lakehouse.EIS.EIS_ROUTING_OPERATION_WH D 
    LEFT JOIN ( SELECT S.TYPE, R.ROUTING_SEQ, R.OPERATION_NO, R.EER_NO, R.PRODUCT_PART_NO
                FROM staging.lakehouse.EIS.EIS_WIP_FA_START_OPERATION_WH S, staging.lakehouse.EIS.EIS_ROUTING_OPERATION_WH R 
...
""")
df.createOrReplaceTempView("MFG_ROUTING_OPERATION_DIM_WH")

count_insert_df = spark.sql(f"""
    SELECT count(1) as total_count
    FROM MFG_ROUTING_OPERATION_DIM_WH s
...
spark.sql(f"""
    WITH max_sequence AS (
        SELECT NVL(MAX(ROUTE_OPERATION_NO_KEY),100000000) AS max_seq FROM {master_table} WHERE INF_SESS_NAME='master_MFG_ROUTING_OPERATION_DIM_WH'
    )
    MERGE INTO {master_table} t 
    USING (SELECT * FROM MFG_ROUTING_OPERATION_DIM_WH) s 
    ON t.OPERATION_NO = s.OPERATION_NO and t.EER_NO = s.EER_NO and t.OPERATION_SEQ=s.OPERATION_SEQ and COALESCE(t.PRODUCT_PART_NO_KEY,0)=COALESCE(s.PRODUCT_PART_NO_KEY,0)
    WHEN NOT MATCHED
