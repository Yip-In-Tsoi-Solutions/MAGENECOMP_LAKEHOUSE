...
df = spark.sql(
    f"""
        SELECT 
            ...
            'EIS_JIT_CELL_WN' as SOURCE_TABLE_NAME,
            {inf_batch_id} as INF_BATCH_ID,
            'master_MFG_LINE_DIM_WN' as INF_SESS_NAME,
        ...
        FROM staging.lakehouse.EIS.EIS_JIT_CELL_WN 
    """
)
df.createOrReplaceTempView("MFG_LINE_DIM_WN")

count_update_df = spark.sql(f"""
    SELECT count(1) as total_count
    FROM {master_table} t 
    INNER JOIN MFG_LINE_DIM_WN s 
...
spark.sql(f"""
    WITH max_sequence AS (
        SELECT NVL(MAX(LINE_NO_KEY),600000000) AS max_seq FROM {master_table} WHERE PLANT='WN'
    )
    MERGE INTO {master_table} t
    USING (SELECT * FROM MFG_LINE_DIM_WN) s
