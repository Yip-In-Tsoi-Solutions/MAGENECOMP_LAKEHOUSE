...
initial_start_date=Variable.get('initial_start_date')
...
df = spark.sql(f"""
    ...
    WHERE A.TRANSFER_DATE BETWEEN cast(cast(date_add(current_date(),DATEDIFF(current_date(),'{initial_start_date}')) as string) || ' 07:00:01' as timestamp)
...
spark.sql(f"""
     DELETE FROM {iceberg_table} WHERE INF_SESS_NAME='process_SFC_WIP_RUNCARD_FACT_WA'
""")
