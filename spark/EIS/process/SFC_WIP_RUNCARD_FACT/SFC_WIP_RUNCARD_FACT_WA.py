...
appName = "SFC_WIP_RUNCARD_FACT_WA"
...
df = spark.sql(f"""
    ...
    FROM staging.lakehouse.EIS.EIS_WIP_FA_WA A
              ,staging.lakehouse.EIS.EIS_WIP_FA_RUNCARD_WA B
              ,staging.lakehouse.EIS.EIS_DOCUMENT_EER_WA C
    WHERE A.TRANSFER_DATE BETWEEN cast(cast(date_add(current_date(),-1) as string) || ' 07:00:01' as timestamp)
    	...
    	AND A.OPERATION_NO NOT IN (  SELECT DISTINCT OPERATION_NO
    			FROM staging.lakehouse.EIS.EIS_WIP_FA_START_OPERATION_WA
    			WHERE ( PRODUCT_PART_NO ,EER_NO ) IN (
    						SELECT DISTINCT PRODUCT_PART_NO ,EERNO
    						FROM staging.lakehouse.EIS.EIS_DOCUMENT_EER_WA
    						WHERE (
    						CREATED_DATE > date_add(current_date(),-1000)
    ...
    SELECT case when M.TRANSFER_DATE >= cast(cast(cast(M.TRANSFER_DATE as date) as string) || ' 07:00:01' as timestamp)
    	...
    	,CASE 	WHEN OPERATION_NO = '480'
    		THEN (
    			SELECT MAX(OPERATION_NO) OPERATION_NO
    			FROM staging.lakehouse.EIS.EIS_WIP_FA_START_OPERATION_WA
    ...
    FROM (
    	...
    	FROM staging.lakehouse.EIS.EIS_WIP_FA_WIP_TRANS_WA A
    		,staging.lakehouse.EIS.EIS_WIP_FA_RUNCARD_WA B
    		,staging.lakehouse.EIS.EIS_DOCUMENT_EER_WA C
    		,staging.lakehouse.EIS.EIS_WIP_FA_WA D
    		,(
    		SELECT OPERATION_NO
    		...
    		FROM staging.lakehouse.EIS.EIS_OPERATION_WA
...
""")
df.createOrReplaceTempView("SFC_WIP_RUNCARD_FACT_WA")

# print("Count record :",df.count())

spark.sql(f"""
     DELETE FROM {iceberg_table} WHERE INF_BATCH_ID={inf_batch_id} AND INF_SESS_NAME='process_SFC_WIP_RUNCARD_FACT_WA'
""")

count_before_insert = spark.table("process.lakehouse.FACT.SFC_WIP_RUNCARD_FACT").filter("INF_SESS_NAME = 'process_SFC_WIP_RUNCARD_FACT_WA'").count()
...
spark.sql(f"""
    ...
    SELECT 
        *,
        ...
        'SFC_WIP_RUNCARD_FACT_WA' as SOURCE_TABLE_NAME,
        {inf_batch_id} as INF_BATCH_ID,
        'process_SFC_WIP_RUNCARD_FACT_WA' as INF_SESS_NAME,
        ...
        FROM SFC_WIP_RUNCARD_FACT_WA
""")

count_after_insert = spark.table("process.lakehouse.FACT.SFC_WIP_RUNCARD_FACT").filter("INF_SESS_NAME = 'process_SFC_WIP_RUNCARD_FACT_WA'").count()
