...
appName = "EIS_WIP_FA_WN"
tableName = f"lakehouse.EIS.EIS_WIP_FA_WN"
...
df = spark.read\
    ...
    .load()
df.createOrReplaceTempView("EIS_WIP_FA_WN")

# count_df = spark.sql(f"""
#     SELECT 
#         count(*) as total_count
#         FROM EIS_WIP_FA_WN 
#         WHERE TRANSFER_DATE BETWEEN cast(cast(date_add(current_date(),-1) as string) || ' 07:00:01' as timestamp)
...
spark.sql(f"""
    DELETE FROM {iceberg_table} WHERE INF_BATCH_ID={inf_batch_id}
""")

count_before_insert = spark.table("staging.lakehouse.EIS.EIS_WIP_FA_WN").count()
...
spark.sql(f"""
    ...
    SELECT 
        *,
        ...
        'EIS_WIP_FA_WN' as SOURCE_TABLE_NAME,
        {inf_batch_id} as INF_BATCH_ID,
        'Staging_EIS_WIP_FA_WN' as INF_SESS_NAME,
...
count_after_insert = spark.table("staging.lakehouse.EIS.EIS_WIP_FA_WN").count()
