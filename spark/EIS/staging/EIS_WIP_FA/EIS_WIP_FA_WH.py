...
appName = "EIS_WIP_FA_WH"
tableName = f"lakehouse.EIS.EIS_WIP_FA_WH"
...
df = spark.read\
    ...
    .load()
df.createOrReplaceTempView("EIS_WIP_FA_WH")
...
count_before_insert = spark.table("staging.lakehouse.EIS.EIS_WIP_FA_WH").count()
...
spark.sql(f"""
    ...
    SELECT 
        *,
        ...
        'EIS_WIP_FA_WH' as SOURCE_TABLE_NAME,
        {inf_batch_id} as INF_BATCH_ID,
        'Staging_EIS_WIP_FA_WH' as INF_SESS_NAME,
...
count_after_insert = spark.table("staging.lakehouse.EIS.EIS_WIP_FA_WH").count()
