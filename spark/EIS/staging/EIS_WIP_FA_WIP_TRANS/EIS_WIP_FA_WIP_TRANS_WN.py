...
appName = "EIS_WIP_FA_WIP_TRANS_WN"
tableName = f"lakehouse.EIS.EIS_WIP_FA_WIP_TRANS_WN"
...
df = spark.read\
    ...
    .load()
df.createOrReplaceTempView("EIS_WIP_FA_WIP_TRANS_WN")

count_df = spark.sql(f"""
    SELECT 
        count(*) as total_count
        FROM EIS_WIP_FA_WIP_TRANS_WN 
...
spark.sql(f"""
    ...
    SELECT 
        *,
        ...
        'EIS_WIP_FA_WIP_TRANS_WN' as SOURCE_TABLE_NAME,
        {inf_batch_id} as INF_BATCH_ID,
        'Staging_EIS_WIP_FA_WIP_TRANS_WN' as INF_SESS_NAME,
