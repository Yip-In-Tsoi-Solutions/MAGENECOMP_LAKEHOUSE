...
appName = "EIS_WIP_FA_RUNCARD_WA"
tableName = f"lakehouse.EIS.EIS_WIP_FA_RUNCARD_WA"
...
df = spark.read\
    ...
    .option("dbtable", "EIS.EIS_WIP_FA_RUNCARD") \
    .option("user", oracleUser) \
    .option("password", oraclePass) \
    .option("driver", "oracle.jdbc.driver.OracleDriver") \
    .load()
df.createOrReplaceTempView("EIS_WIP_FA_RUNCARD_WA")
...
spark.sql(f"""
    ...
    SELECT 
        *,
        ...
        'Staging_eis.eis_wip_fa_runcard_wa' as INF_SESS_NAME,
        ...
        FROM EIS_WIP_FA_RUNCARD_WA
        WHERE CREATED_DATE BETWEEN cast(cast(date_add(current_date(),-1) as string) || ' 07:00:01' as timestamp)
