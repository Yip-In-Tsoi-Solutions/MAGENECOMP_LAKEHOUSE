...
appName = "DATA_CON_TRANSACTION_ADHOC"
...
# spark.sql(f"DELETE FROM mart.lakehouse.DATA_CON_TRANSACTION_TEST WHERE MachineNo = '{machine_name_rerun}' AND Date = cast('{date_rerun}' as date)")
spark.sql(f"DELETE FROM mart.lakehouse.DATA_CON_TRANSACTION WHERE Date = to_date('{dtc_file_date}','yyyyMMdd')")
...
df = spark.sql(f""" 
      ...
      where a.Date = to_date('{dtc_file_date}','yyyyMMdd')
