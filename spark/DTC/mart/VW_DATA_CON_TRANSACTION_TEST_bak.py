...
appName = "DATA_CON_TRANSACTION_TEST"
...
# spark.sql(f"DELETE FROM mart.lakehouse.DATA_CON_TRANSACTION_TEST WHERE Date = date_add(current_date(),-1)")
...
df = spark.sql(f""" 
                with cte1 as (
                        ...
                        on b.MachineNo=a.MachineNo and upper(b.VariableName)=upper(a.VariableName) 
                        where a.Date = date_add(current_date(),-1)
...
df.createOrReplaceTempView("DATA_CON_TRANSACTION_TEST_tmp")

spark.sql(f"""
          INSERT INTO mart.lakehouse.DATA_CON_TRANSACTION_TEST SELECT * FROM DATA_CON_TRANSACTION_TEST_tmp
""")