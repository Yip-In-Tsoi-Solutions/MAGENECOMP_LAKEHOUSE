...
appName = "STAGING_DTC_TRANSACTION_ADHOC"
...
dtc_num = Variable.get("dtc_num")
dtc_file_date = Variable.get("dtc_file_date")
# inf_batch_id = Variable.get("INF_BATCH_ID")

#sources Table
staging_endpoint = "s3a://staging"
source_path = f"s3a://staging/DTC/ALL_DTC/{dtc_num}*_log-{dtc_file_date}*.txt"
#Destination Table
...
# spark.sql(f" DELETE FROM {iceberg_table} WHERE upper(substr(a.MACHINENAME,1,6)) = '{machine_name_rerun}' AND cast(COLLECTEDDATE as date) = cast('{date_rerun}' as date) ")
...
df = spark.read.format("text").load(source_path)
...
final_df = exploded_df.select(
    ...
    col("json_data.CollectedDate").alias("COLLECTEDDATE"),
    col("json_data.CEID"),
    ...
    col("exploded_values.Value").alias("VALUES")
        ...
        "SOURCE_TABLE_NAME", regexp_replace(input_file_name(),"s3a://staging/DTC/ALL_DTC/","")
    ).withColumn(
        ...
        "DM_MODIFIED_DATE", expr("current_timestamp()")
    ).withColumn(
        "DM_CREATED_BY",lit("Admin")
    ).withColumn(
        "DM_CREATED_DATE", expr("current_timestamp()")
    ).withColumn(
        "DM_DATE_FROM", expr("current_timestamp()")
    ).withColumn(
        "DM_DATE_TO", expr("current_timestamp()")
    )
# final_df.printSchema()

final_df.createOrReplaceTempView("STAGING_DTC_TRANSACTION_ADHOC")
...
spark.sql(f"""
    ...
    SELECT 
        *
    FROM STAGING_DTC_TRANSACTION_ADHOC
""")
