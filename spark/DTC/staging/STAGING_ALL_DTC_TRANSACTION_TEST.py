...
source_path = f"s3a://staging/DTC/ALL_DTC_TEST/*.txt"
#Destination Table
iceberg_table =  f"staging.lakehouse.DTC.STAGING_DTC_TRANSACTION_TEST"
...
final_df = exploded_df.select(
    ...
    col("exploded_values.Value").alias("VALUES")
        ...
        "SOURCE_TABLE_NAME", regexp_replace(input_file_name(),"s3a://staging/DTC/ALL_DTC_TEST/","")
    ).withColumn(
...
query = final_df.writeStream \
    .format("iceberg") \
    .outputMode("append") \
    .option("checkpointLocation", f"s3a://staging/checkpoint/STAGING_ALL_DTC_TRANSACTION_TEST") \
