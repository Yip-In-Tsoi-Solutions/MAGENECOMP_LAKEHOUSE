...
appName = "FACT_DATA_CON_TRANSACTION_TEST"
...
iceberg_table = f"process.lakehouse.DTC.FACT_DATA_CON_TRANSACTION_TEST"
spark = (
...
df = spark.sql(f""" 
    ...
    from  staging.lakehouse.DTC.STAGING_DTC_TRANSACTION_TEST a left join master.lakehouse.DTC.DATA_CON_PARAM_SPEC b on upper(substr(a.MACHINENAME,1,6)) = upper(b.MachineNo)
    and a.NAME = b.VariableName
    WHERE 1=1 
    AND a.DM_CREATED_DATE > (select max(DM_CREATED_DATE) from process.lakehouse.DTC.FACT_DATA_CON_TRANSACTION_TEST)
    AND a.DM_CREATED_DATE <= cast(substring(cast(current_timestamp() as string),0,17) || '00' as timestamp)
"""
)
df.createOrReplaceTempView("SFC_DATA_CON_TRANSACTION_TEST")
...
spark.sql(f"INSERT INTO {iceberg_table} SELECT * FROM SFC_DATA_CON_TRANSACTION_TEST")
