...
appName = "FACT_DATA_CON_TRANSACTION_ADHOC"
...
spark.sql(f""" 
          DELETE FROM {iceberg_table} 
          WHERE 1=1
          AND Date = to_date('{dtc_file_date}','yyyyMMdd') 
          """)
...
df = spark.sql(f""" 
    ...
    ,case when (b.SpecUCL is null or b.SpecUCL = '') then 0 else b.SpecUCL end as SpecUCL,
                a.REC_ID, 
               'STAGING_DTC_TRANSACTION' as SOURCE_TABLE_NAME, 
               {inf_batch_id} as INF_BATCH_ID, 
               'process_FACT_DATA_CON_TRANSACTION_ADHOC' as INF_SESS_NAME, 
    ...
    from  staging.lakehouse.DTC.STAGING_DTC_TRANSACTION a left join master.lakehouse.DTC.DATA_CON_PARAM_SPEC b on upper(substr(a.MACHINENAME,1,6)) = b.MachineNo
    and lower(a.NAME) = lower(b.VariableName)
    WHERE 1=1 
    AND cast(a.COLLECTEDDATE as date) = to_date('{dtc_file_date}','yyyyMMdd')
"""
)
df.createOrReplaceTempView("SFC_DATA_CON_TRANSACTION_tmp")

count_before_insert = spark.table(iceberg_table).filter("SOURCE_TABLE_NAME='STAGING_DTC_TRANSACTION'").count()
...
count_after_insert = spark.table(iceberg_table).filter("SOURCE_TABLE_NAME='STAGING_DTC_TRANSACTION'").count()
